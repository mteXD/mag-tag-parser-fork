...
/**
 * @param c Checks if c can be a part of a variable identifier (letter, number
 * or '_' or '.')
 */
...
/**
 * @param file_path Takes file path as a string, opens it as a stream of
 * characters and tokenizes the file.
 */
    ...
    while (infile.get(c)) {
        ...
        if (is_whitespace(c)) {
            ...
            while (infile.get(c) && c != '"') {
                if (c == '\n') {
                    std::ostringstream oss;
                    oss << "String literal does not end before the end of the "
                           "line! Location: "
                        << line << "," << column;
                    throw std::runtime_error(oss.str());
                }
            ...
            std::ostringstream oss;
            oss << "Failed to parse at character '" << c
                << "', location: " << line << ", " << column;
            throw std::runtime_error(oss.str());
        }
    }
