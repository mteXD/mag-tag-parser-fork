...
class ast_source_t : public ast_node_t {
  ...
  public:
    ast_source_t() {}
    void add_decl(const std::shared_ptr<ast_decl_t> &d) { decls.push_back(d); }
    std::vector<std::shared_ptr<ast_decl_t>> &get_decls() { return decls; }
    void print() {
        for (auto &d : decls) {
            d.get()->print();
        }
...
class ast_decls_t : public ast_node_t {
  ...
  public:
    ast_decls_t() {};
    std::vector<std::shared_ptr<ast_decl_t>> &get_decls() { return decls; }
    void add(const std::shared_ptr<ast_decl_t> &d) { decls.push_back(d); }
    void print() {
        for (auto &d : decls) {
            d.get()->print();
        }
...
class ast_tag_t : public ast_expr_t {
  ...
  public:
    ...
    ~ast_tag_t() {}
    std::string &get_name() { return name; }
    void print() { std::cout << "\tTag '" << name << "'" << std::endl; }
};
...
class ast_edge_t : public ast_node_t {
  public:
    ...
    std::shared_ptr<ast_tag_t> &get_source() { return source; }
    std::shared_ptr<ast_tag_t> &get_end() { return end; }
...
class ast_topology_basic_t : public ast_topology_t {
  public:
    void add_edge(const std::shared_ptr<ast_edge_t> &e) { edges.push_back(e); }
    std::vector<std::shared_ptr<ast_edge_t>> get_edges() { return edges; }
    void print() {
...
class ast_topology_linear_t : public ast_topology_t {
  public:
    void add_tag(const std::shared_ptr<ast_tag_t> &t) { tags.push_back(t); }
    std::vector<std::shared_ptr<ast_tag_t>> &get_tags() { return tags; }
    void print() {
...
class ast_topology_expr_t : public ast_topology_t {
  public:
    ...
    std::shared_ptr<ast_expr_t> &get_expr() { return expr; }
...
class ast_expr_bin_t : public ast_expr_t {
  public:
    enum class Oper { SUM, MUL };
    ast_expr_bin_t(
        const Oper o, const std::shared_ptr<ast_expr_t> &l,
        const std::shared_ptr<ast_expr_t> &r
    )
        : oper(o), lhs(l), rhs(r) {}

    void set_lhs(const std::shared_ptr<ast_expr_t> &l) { lhs = l; }
    std::shared_ptr<ast_expr_t> &get_lhs() { return lhs; }

    void set_rhs(const std::shared_ptr<ast_expr_t> &r) { rhs = r; }
    std::shared_ptr<ast_expr_t> &get_rhs() { return rhs; }

    Oper get_oper() { return oper; }
...
std::shared_ptr<ast_node_t>
    ast_construct(const dertree_t &node, std::shared_ptr<ast_node_t> arg);
